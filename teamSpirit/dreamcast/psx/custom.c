...
void customDrawSortedPrimitives(int depth)
{
	...
	while(primsleft)
	{
		if (opcd==0)
		{
			opcd = (TMD_P_GT4I*)sorts[sortBucket++];
			continue;
		}
...
//				BEGINPRIM(si, POLY_FT3);
				...
				*(unsigned long *)  (&si->u0) = *(unsigned long *) (&op->tu0);		// Texture coords
				*(unsigned long *)  (&si->u1) = *(unsigned long *) (&op->tu1);
				*(unsigned long *)  (&si->u2) = *(unsigned long *) (&op->tu2);
				...
				switch (lightmode)
				{
					...
					case DIRECTIONONLY:
						...
						break;
					case COLOURIZE:
						*(unsigned long *)  (&si->r0) = *(unsigned long *) (&modctrl->col);
						break;
					default:
						*(unsigned long *) (&si->r0) = *(unsigned long *) (&op->r0);
		 		}

				setPolyFT3(si);
				si->code = op->cd | modctrl->semitrans;
//				ENDPRIM(si, depth, POLY_FT3);
				op = (TMD_P_FT3I*)op->next;
				break;
#undef si
#undef op
/*-----------------------------------------------------------------------------------------------------------------*/
...
//				BEGINPRIM(si, POLY_FT4);
				...
				*(unsigned long *)  (&si->x3) = *(unsigned long *) (&tfv[op->v3]);
				...
				switch (lightmode)
				{
					...
					case COLOURIZE:
						*(unsigned long *)  (&si->r0) = *(unsigned long *) (&modctrl->col);
						break;
					default:
						*(unsigned long *) (&si->r0) = *(unsigned long *) (&op->r0);		// 9 cycles here
				}
				*(unsigned long *)  (&si->u2) = *(unsigned long *) (&op->tu2);
				*(unsigned long *)  (&si->u3) = *(unsigned long *) (&op->tu3);

				setPolyFT4(si);
				si->code = op->cd | modctrl->semitrans;
// 				ENDPRIM(si, depth, POLY_FT4);
				op = (TMD_P_FT4I*)op->next;
				break;
#undef si
#undef op
/*-----------------------------------------------------------------------------------------------------------------*/
...
//				BEGINPRIM(si, POLY_GT3);
				...
				*(unsigned long *)  (&si->u2) = *(unsigned long *) (&op->tu2);
				...
				switch (lightmode)
				{
					...
					case COLOURIZE:
						*(unsigned long *)  (&si->r0) = *(unsigned long *) (&modctrl->col);
						*(unsigned long *)  (&si->r1) = *(unsigned long *) (&modctrl->col);
						*(unsigned long *)  (&si->r2) = *(unsigned long *) (&modctrl->col);
						break;
					default:
						*(unsigned long *)  (&si->r0) = *(unsigned long *) (&op->r0);
						*(unsigned long *)  (&si->r1) = *(unsigned long *) (&op->r1);
						*(unsigned long *)  (&si->r2) = *(unsigned long *) (&op->r2);
				}
				setPolyGT3(si);
				si->code = op->cd | modctrl->semitrans;
//				ENDPRIM(si, depth, POLY_GT3);
				op = (TMD_P_GT3I*)op->next;
				break;
#undef si
#undef op
/*-----------------------------------------------------------------------------------------------------------------*/
				...
				*(unsigned long *)  (&si->x3) = *(unsigned long *) (&tfv[op->v3]);
				...
				switch (lightmode)
				{
					...
					case COLOURIZE:
						...
						*(unsigned long *)  (&si->r1) = *(unsigned long *) (&modctrl->col);
						*(unsigned long *)  (&si->r2) = *(unsigned long *) (&modctrl->col);
						*(unsigned long *)  (&si->r3) = *(unsigned long *) (&modctrl->col);
						break;
					default:
						*(unsigned long *)  (&si->r0) = *(unsigned long *) (&op->r0);
						*(unsigned long *)  (&si->r1) = *(unsigned long *) (&op->r1);
						*(unsigned long *)  (&si->r2) = *(unsigned long *) (&op->r2);
						*(unsigned long *)  (&si->r3) = *(unsigned long *) (&op->r3);
				}
		
				setPolyGT4(si);
				si->code = op->cd | modctrl->semitrans;
// 				ENDPRIM(si, depth, POLY_GT4);
				op = (TMD_P_GT4I*)op->next;
				break;
...
#define op ((TMD_P_FT3I*)opcd)
			...
			case GPU_COM_TF3:
				...
				*(u_long *)  (&si->u1) = *(u_long *) (&op->tu1);
				...
				setPolyFT3(si);
				//si->code = op->cd | modctrl->semitrans;
//bb			ENDPRIM(si, ((sortBucket+minDepth) >> 2) & 1023, POLY_FT3);
				ENDPRIM(si, depth, POLY_FT3);
...
#define op ((TMD_P_FT4I*)opcd)
				...
				*(u_long *)  (&si->u3) = *(u_long *) (&op->tu3);
				...
				setPolyFT4(si);
				//si->code = op->cd | modctrl->semitrans;
//bb				ENDPRIM(si, ((sortBucket+minDepth) >> 2) & 1023, POLY_FT4);
 				ENDPRIM(si, depth, POLY_FT4);
				op = op->next;
				break;
#undef si
...
#define op ((TMD_P_GT3I*)opcd)
			...
			case GPU_COM_TG3:
				...
				*(u_long *)  (&si->u2) = *(u_long *) (&op->tu2);
				...
				setPolyGT3(si);
				//si->code = op->cd | modctrl->semitrans;
//bb			ENDPRIM(si, ((sortBucket+minDepth) >> 2) & 1023, POLY_GT3);
				ENDPRIM(si, depth, POLY_GT3);
				op = op->next;
				break;
#undef si
...
#define op opcd
			...
			case GPU_COM_TG4:
				...
				*(u_long *)  (&si->x3) = *(u_long *) (&tfv[op->v3]);
				...
				setPolyGT4(si);
				//si->code = op->cd | modctrl->semitrans;
//bb			ENDPRIM(si, ((sortBucket+minDepth) >> 2) & 1023, POLY_GT4);
 				ENDPRIM(si, depth, POLY_GT4);
				(int)op = op->next;
				break;
...
#define op ((TMD_P_FG4I*)opcd)
			...
			case GPU_COM_F4:
				...
				*(u_long *)  (&si->x3) = *(u_long *) (&tfv[op->v3]);
				...
				setPolyG4(si);
				//si->code = op->cd | modctrl->semitrans;
//bb			ENDPRIM(si, ((sortBucket+minDepth) >> 2) & 1023, POLY_G4);
 				ENDPRIM(si, depth, POLY_G4);
				op = op->next;
				break;
#undef si
...
#define op ((TMD_P_FG3I*)opcd)
			...
			case GPU_COM_F3:
				...
				*(u_long *)  (&si->r0) = *(u_long *) (&op->r0);
				...
				setPolyG3(si);
				//si->code = op->cd | modctrl->semitrans;
//bb			ENDPRIM(si, ((sortBucket+minDepth) >> 2) & 1023, POLY_G3);
				ENDPRIM(si, depth, POLY_G3);
				op = op->next;
				break;
#undef si
