...
//	}
	...
	//test !!!!!
	dptr = (long*)transformedDepths;
	dptr2 = (float*)transformedDepths2;
	for(i = 0; i < mesh->n_verts; i++)
	{
		tdepth = *dptr;
		*dptr++ = tdepth >> 2;
//		*dptr2++ = 1.0 / (float)tdepth;		
	}
...
//ma				addPrimLen(ot+(depth), (si),12,t2);
...
//			if(clipflag < 0)
			{
// Put the polygon into the OT (Note that we re-use "depth" as one of the "t" variables)
//ma				addPrimLen(ot+(depth), (si),9,t2);
// Copy the polygon data over into the OT
/*
#ifdef TEMP_STORE
				t1 = *(u_long *) (&op->u0);
...
//			else
//			{
//				if((*mesh)->name_crc == utilStr2CRC("LAWNF")/*||
...
//	sprintf(tbuffer,"%d => %d",removed_models,world->n_meshes);
//	fontPrint(font, 20,20, tbuffer, 255,255,255);
