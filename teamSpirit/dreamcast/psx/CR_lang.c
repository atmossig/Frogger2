...
KMSTRIPCONTEXT	flagStripContext;
KMSTRIPHEAD		flagStripHead;
KMVERTEX_03		flagVertices[] =
{
...
*/
	for(loop1 = 0; loop1 < FLAGPOLYS; loop1 ++)
	{
		...
		{
			xy = screenXYs[loop1][loop2];
			flagVertices[0].fX = (float)((short)(xy&0xFFFF));
			flagVertices[0].fY = (float)((short)(xy>>16));
			flagVertices[0].u.fZ = 1.0;
			flagVertices[0].fU = textureU[loop1][loop2][0];
			flagVertices[0].fV = textureV[loop1][loop2][0];
			flagVertices[0].uBaseRGB.dwPacked = RGBA(flagShade[loop1][loop2],flagShade[loop1][loop2],flagShade[loop1][loop2],255);

			xy = screenXYs[loop1][loop2 + 1];
			flagVertices[1].fX = (float)((short)(xy&0xFFFF));
			flagVertices[1].fY = (float)((short)(xy>>16));
			flagVertices[1].u.fZ = 1.0;
			flagVertices[1].fU = textureU[loop1][loop2][1];
			flagVertices[1].fV = textureV[loop1][loop2][1];
			flagVertices[1].uBaseRGB.dwPacked = RGBA(flagShade[loop1][loop2 + 1],flagShade[loop1][loop2 + 1],flagShade[loop1][loop2 + 1],255);

			xy = screenXYs[loop1 + 1][loop2];
			flagVertices[2].fX = (float)((short)(xy&0xFFFF));
			flagVertices[2].fY = (float)((short)(xy>>16));
			flagVertices[2].u.fZ = 1.0;
			flagVertices[2].fU = textureU[loop1][loop2][2];
			flagVertices[2].fV = textureV[loop1][loop2][2];
			flagVertices[2].uBaseRGB.dwPacked = RGBA(flagShade[loop1 + 1][loop2],flagShade[loop1 + 1][loop2],flagShade[loop1 + 1][loop2],255);
	
			xy = screenXYs[loop1 + 1][loop2 + 1];
			flagVertices[3].fX = (float)((short)(xy&0xFFFF));
			flagVertices[3].fY = (float)((short)(xy>>16));
			flagVertices[3].u.fZ = 1.0;
			flagVertices[3].fU = textureU[loop1][loop2][3];
			flagVertices[3].fV = textureV[loop1][loop2][3];
			flagVertices[3].uBaseRGB.dwPacked = RGBA(flagShade[loop1 + 1][loop2 + 1],flagShade[loop1 + 1][loop2 + 1],flagShade[loop1 + 1][loop2 + 1],255);
	
			kmChangeStripTextureSurface(&flagStripHead,KM_IMAGE_PARAM1,DCKtextureList[0].surfacePtr);

			kmStartStrip(&vertexBufferDesc, &flagStripHead);		
			kmSetVertex(&vertexBufferDesc, &flagVertices[0], KM_VERTEXTYPE_03, sizeof(KMVERTEX_03));
			kmSetVertex(&vertexBufferDesc, &flagVertices[1], KM_VERTEXTYPE_03, sizeof(KMVERTEX_03));
			kmSetVertex(&vertexBufferDesc, &flagVertices[2], KM_VERTEXTYPE_03, sizeof(KMVERTEX_03));	
			kmSetVertex(&vertexBufferDesc, &flagVertices[3], KM_VERTEXTYPE_03, sizeof(KMVERTEX_03));	
...
void languageInitialise()
{
	...
	// initialise flag strip context and head
	memset(&flagStripContext,0,sizeof(flagStripContext));
	memset(&flagStripHead,0,sizeof(flagStripHead));
	flagStripContext.nSize = sizeof(KMSTRIPCONTEXT);
	kmInitStripContext(KM_STRIPCONTEXT_SYS_GOURAUD | KM_TRANS_POLYGON, &flagStripContext);
	flagStripContext.StripControl.nUserClipMode	 						= KM_USERCLIP_DISABLE;
	flagStripContext.StripControl.nShadowMode			 				= KM_NORMAL_POLYGON;
	flagStripContext.StripControl.bGouraud		 						= KM_TRUE;
	flagStripContext.ObjectControl.nDepthCompare			 			= KM_ALWAYS;
	flagStripContext.ObjectControl.nCullingMode			 				= KM_NOCULLING;
	flagStripContext.ObjectControl.bZWriteDisable						= KM_FALSE;
	flagStripContext.ImageControl[KM_IMAGE_PARAM1].nSRCBlendingMode		= KM_SRCALPHA;
	flagStripContext.ImageControl[KM_IMAGE_PARAM1].nDSTBlendingMode		= KM_INVSRCALPHA;
	flagStripContext.ImageControl[KM_IMAGE_PARAM1].bSRCSelect			= KM_FALSE;
	flagStripContext.ImageControl[KM_IMAGE_PARAM1].bDSTSelect			= KM_FALSE;
	flagStripContext.ImageControl[KM_IMAGE_PARAM1].nFogMode				= KM_NOFOG;
	flagStripContext.ImageControl[KM_IMAGE_PARAM1].bColorClamp			= KM_FALSE;	
	flagStripContext.ImageControl[KM_IMAGE_PARAM1].bUseAlpha			= KM_TRUE;
	flagStripContext.ImageControl[KM_IMAGE_PARAM1].bIgnoreTextureAlpha	= KM_FALSE;
	flagStripContext.ImageControl[KM_IMAGE_PARAM1].nFilterMode			= KM_POINT_SAMPLE;
	kmGenerateStripHead03(&flagStripHead,&flagStripContext);
...
//	textureDownloadBank(languageData.flagTextures);
	...
	for (loop = 0; loop < LANG_NUMLANGS; loop++)
		languageData.flagTexture[loop] = FindTexture(flagName[loop]);
...
//ma		fontDispSprite(FindTexture(buttonName[0]),-200, 75,128,128,128,font->alpha);
//ma		fontDispSprite(FindTexture(buttonName[1]), 180, 75,128,128,128,font->alpha);
//ma		fontDispSprite(FindTexture(buttonName[2]), -16, 90,128,128,128,font->alpha);
...
//ma	textureUnloadBank(languageData.flagTextures);
	FreeAllTextureBanks();
