...
PSIOBJECT *psiObjectScan(PSIOBJECT *obj, char *name)
{
	...
	while(obj)
	{
		// *ASL* 21/07/2000 - Cast obj->meshdata->name from (UBYTE *) to (char *)
		utilUpperStr((char *)obj->meshdata->name);
		if(strcmp((char *)obj->meshdata->name, name) == 0)
			return obj;
...
void psiFixupMesh(PSIMESH *mesh)
{
	...
	// *ASL* 21/07/2000 - Re-cast to SQKEYFRAME rather than SVKEYFRAME
...
void *psiLoadPIL(char *pilName)
{
	...
	i = (int)*addr;
	// *ASL* 21/07/2000 Ambig - Re-cast from LHS (char *) to RHS (int *)
	table = (int *) (addr+4);
