...
// *ASL* 21/07/2000 - Needed to determine the build target
#include "prefix_dc.h"
...
//			SetUpWaterMesh ( newItem->bffActor );
		...
		if ( !newItem->bffActor )
		{
			...
			{
				utilPrintf ( "Could not load file %s\n", newname );
				utilPrintf ( "Although I could Not Find It, It could also Be A New BFF Actor, So I'm going to Carry On!!!!");
				//return NULL;
			}
			else
			{
				...
				{
//ma					newItem->actor->psiData.flags |= PSIACTOR_TRANSPARENT;
//ma					newItem->actor->psiData.flags |= ACTOR_DYNAMICSORT;
				}
			}
		}
...
// *ASL* 21/07/2000 - Changed from 'else PSX_VERSION' to '#endif #ifdef PSX_VERSION'
#endif
#ifdef PSX_VERSION
	gstrupr(newname);
