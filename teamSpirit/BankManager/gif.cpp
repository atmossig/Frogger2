...
GIFF *currGIF;
int   bad_code_count = 0, GIFlineCount, GIFscrWidth, GIFscrHeight;
char *GIFscrPtr;
...
int getGIF8Info(char *filename, int *width, int *height)
{
	...
	if ((currGIF = openGIF(filename))==NULL)
		return 0;

	*width = currGIF->h.width;
	*height = currGIF->h.height;
	numImages = currGIF->numImages;
...
/*	--------------------------------------------------------------------------------
	...
	Info 		:
*/
int loadGIF8(char *filename, char *srcPtr, char *palPtr)
{
  short loop;
  ...
  GIFscrPtr = srcPtr;
  GIFscrWidth = currGIF->h.width;
  GIFscrHeight = currGIF->h.height;
  GIFlineCount = 0;


//  setPalette(0, 256, currGIF->palette);
  for(loop=0;loop<256*3;loop++)
    palPtr[loop] = currGIF->palette[loop];

  decoder(currGIF->h.width);
  closeGIF(currGIF);
  return 1;
...
GIFF *openGIF(char *fName)
{
  GIFF  *gif;
  short  i;
  char   smallBuf[256*3], c;


  gif = (GIFF *)malloc(sizeof(GIFF));
  gif->colorMap = 0;
  if ((gif->fp = fopen(fName, "rb"))==NULL)
    {
//	   closeGIF(gif);
		return NULL;
	 }
  fread(&(gif->h), sizeof(GIFH), 1, gif->fp);

  if (strncmp(gif->h.id, "GIF87a", 6))
    {
	   closeGIF(gif);
		return NULL;
	 }

  gif->colorRes = ((gif->h.colorInfo>>4) & 7)+1;
  gif->colorDepth = (gif->h.colorInfo & 7)+1;
  gif->colorMap = (gif->h.colorInfo>>7) & 1;

  if (gif->colorMap)
    {
	   gif->palette = (unsigned char *)malloc(256*3);
		fread(gif->palette, 3*(1<<(gif->colorDepth)), 1, gif->fp);
      for(i=0; i<256; i++)
		  {
		    gif->palette[i*3+0] = gif->palette[i*3+0]/4;
		    gif->palette[i*3+1] = gif->palette[i*3+1]/4;
		    gif->palette[i*3+2] = gif->palette[i*3+2]/4;
		  }
	 }

  fread(smallBuf, 9, 1, gif->fp); // Skip image desc for now

  c = getc(gif->fp);
  if (c & 0x80)                   // Local color map
    {
  		fread(gif->palette, 3*(1<<((c&7)+1)), 1, gif->fp);
      for(i=0; i<256; i++)
		  {
		    gif->palette[i*3+0] = gif->palette[i*3+0]/4;
		    gif->palette[i*3+1] = gif->palette[i*3+1]/4;
  ...
  c = getc(gif->fp);
	gif->numImages = 1;
  while(c == 0x21)               // Attempt to skip rubbish blocks
    {
	   getc(gif->fp);
		i = getc(gif->fp);
		fread(smallBuf, i+4, 1, gif->fp);
		c = getc(gif->fp);
		gif->numImages++;
	 }
  ungetc(c, gif->fp);
...
short get_byte()
{
  return getc(currGIF->fp);  
}


short out_line(unsigned char *pixels, int linelen)
{
  int   i;
  char *ptr;

  if (GIFlineCount<GIFscrHeight)
    {
      ptr = GIFscrPtr;
      for(i=0;((i<GIFscrWidth)&&(i<linelen));i++)
        *ptr++ = *pixels++;
      GIFscrPtr += GIFscrWidth;
      GIFlineCount++;
		return 0;
	 }
  else
