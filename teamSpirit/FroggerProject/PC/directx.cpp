...
//bb?
//bb
//fixed fStart = 0.3*SCALE;
//fixed fEnd = 0.6*SCALE;
fixed fStart = ToFixed(0.3*SCALE);
fixed fEnd = ToFixed(0.6*SCALE);
...
float TriangleArea(float x1,float y1,float x2,float y2,float x3,float y3)
{
	...
	lAs = fl_MagnitudeSquared(&a);
	lBs = fl_MagnitudeSquared(&b);
	lCs = fl_MagnitudeSquared(&c);
...
//bb
//	fogAmt = FOGADJ(z);
	fogAmt = (1.0-((z-(fStart/4096.0F))*(fEnd/4096.0F)));
	if (fogAmt<0)
		fogAmt=0;
	if (fogAmt>1)
		fogAmt=1;
