...
fixed gameSpeed = ToFixed(1);
char dprintbuf[255] = "---";

HBITMAP appBackgnd;

WININFO winInfo;
//mm BYTE lButton = 0, rButton = 0;
u8 lButton = 0, rButton = 0;
int runQuit = 0;
...
char baseDirectory[MAX_PATH] = "c:\\teamspirit\\pcversion\\";
char editorOk = 0;
long drawTimers = 0;
float keyDelay;
...
//					camSideOfs+=2*gameSpeed;
					camSideOfs+=(gameSpeed*2);
				else
//bb
//					camDist.v[2]+=2*gameSpeed;
					camDist.v[2]+=(gameSpeed*2)>>12;
			}
...
//				gameSpeed = (newTickCount-actTickCount)/(1000.0/60.0);
				gameSpeed = ToFixed((newTickCount-actTickCount)/(1000.0/60.0));
...
*/
FLVECTOR oldCCSource, oldCCTarget;
...
//		DrawEditor();
	...
	SetVectorFlF(&oldCCSource, &currCamSource[screenNum]);
	SetVectorFlF(&oldCCTarget, &currCamTarget[screenNum]);
	...
	SetVectorFV(&currCamSource[screenNum], &zero);
	SetVectorFF(&currCamTarget[screenNum], &inVec);
	
	
	// Restore currCam vectors
	SetVectorFFl(&currCamSource[screenNum], &oldCCSource);
	SetVectorFFl(&currCamTarget[screenNum], &oldCCTarget);
...
//				sprintf(speed,"%.2f fps - %lu polys (%lu) - %lu sprites- %lu pixels",(60.0/gameSpeed),numFacesDrawn,totalFacesDrawn,numSprites,numPixelsDrawn);
				sprintf(speed,"%.2f fps - %lu polys (%lu) - %lu sprites- %lu pixels",(60.0/(gameSpeed/4096.0F)),numFacesDrawn,totalFacesDrawn,numSprites,numPixelsDrawn);
