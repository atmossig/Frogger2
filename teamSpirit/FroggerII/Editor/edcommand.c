...
void ToolbarSelect(int command)
{
	...
	case TB_MAKECAMERA_FOLLOWFROG:
		if (selectionList)
		{
			EditorAddCameraCase(selectionList, LOOK_AT_FROG, &camDist);
			UpdateSelection( );
		}
		break;
	...
	case TB_MAKECAMERA_FOLLOWFROG_NOTILT:
		if (selectionList)
		{
			EditorAddCameraCase(selectionList, LOOK_AT_FROG_NOTILT, &camDist);
			UpdateSelection( );
		}
		break;
	...
	case TB_MAKECAMERA_WATCHFROG:
		if (selectionList)
		{
			EditorAddCameraCase(selectionList, FIXED_SOURCE, &currCamSource);
			UpdateSelection( );
		}
		break;
	...
	case TB_MAKECAMERA_STATIC:
		if (selectionList)
		{
			EditorAddCameraCase(selectionList, STATIC_CAMERA, &currCamSource);
			UpdateSelection( );
		}
		break;
	...
	case TB_MAKECAMERA_WATCHTILE:
		if (selectionList)
		{
			EditorAddCameraCase(selectionList, LOOK_AT_TILE, &currCamSource);
			UpdateSelection( );
		}
		break;
	...
	case TB_MAKECAMERA_LOOKDIR:
		...
		{
			VECTOR dir;
			CREATEENTITY *cam = EditorAddCameraCase(selectionList, LOOK_IN_DIR, &currCamSource);
			SubVector( &dir, &currCamTarget, &currCamSource );
			MakeUnit( &dir );
			cam->startNode = 0;
			cam->startNode |= ((char)(dir.v[0]*0x7F))&0x000000FF;
			cam->startNode |= (((char)(dir.v[1]*0x7F))<<8)&0x0000FF00;
			cam->startNode |= (((char)(dir.v[2]*0x7F))<<16)&0x00FF0000;
			UpdateSelection( );
		}
		break;
...
void EditorUndo()
{
	...
	if (editPath && editPath->nodes)
	{
		if (editPath->nodes->link)
		{
			...
			prev->link = NULL; JallocFree((UBYTE**)&node);
		}
		else
		{
			JallocFree((UBYTE**)&editPath->nodes);
			editPath->nodes = NULL;
		}
	}
}
...
void CopyEntitySelection( EDITGROUP *group )
{
	...
	if( !copyEntity )
		copyEntity = (CREATEENTITY *)JallocAlloc( sizeof(CREATEENTITY),YES,"copyEntity" );
...
void SetSelection(CREATEENTITY *entity, VECTOR *iconPos)
{
...
void PlaceEnemy(EDITPATHNODE *node)
{
	...
	SetSelection(e, &node->tile->centre);
}
