...
HRESULT SendChatMessage(LPSTR string, LONG lStrLen )
{
	...
	// get item text
	cmemcpy(lpszChatStr,string,lStrLen);
	...
	cmemcpy(lpChatMessage->szMsg,lpszChatStr,lStrLen);
...
void AddChatMessage( char *msg )
{
	...
	cstring = (CHATSTRING *)JallocAlloc(sizeof(CHATSTRING),YES,"ChatString");
	cstring->msgLen = len;
	cstring->msg = (char *)JallocAlloc(len,YES,"string");

	cmemcpy( cstring->msg, msg, len );
...
void TruncateChatBuffer( )
{
	...
	while( chatBuffer.numEntries > chatBuffer.maxEntries )
	{
		for( cstring = chatBuffer.head.next; ; cstring = cstring->next )
		{
			...
			{
				if( cstring->next->msg )
					JallocFree( (UBYTE **)&cstring->next->msg );

				JallocFree( (UBYTE **)&cstring->next );
				cstring->next = NULL;
				chatBuffer.numEntries--;
				break;
			}
